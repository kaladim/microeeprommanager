...
public:
    ...
    bool IsOwnAreaWrittenOnly(uint8_t block_id, std::vector<uint8_t> &eeprom_before_write)
    {
        ...
        if (not std::equal(eeprom_before_write.begin(),
                           eeprom_before_write.begin() + block_cfg->offset_in_eeprom,
                           eep_sim->eeprom.begin()))
        {
            return false;
        }
        ...
        return std::equal(eeprom_before_write.begin() + block_cfg->offset_in_eeprom + block_area_size_in_eeprom,
                          eeprom_before_write.end(),
    ...
    std::vector<uint8_t> FilterBlocksByManagementTypeAndInstanceCount(uint8_t block_management_type, uint8_t instance_count)
    {
        ...
        for (uint8_t i = 0; i < MEEM_BLOCK_COUNT; i++)
        {
            if ((MEEM_block_config[i].management_type == block_management_type) and
                (MEEM_block_config[i].instance_count == instance_count))
            {
                blocks_ids.push_back(i);
            }
        }
    ...
    std::vector<uint8_t> GenerateRandomBytes(size_t length)
    {
        ...
        std::generate(random_bytes.begin(),
                      random_bytes.end(),
                      [&]()
